...
| Tests | CLI repo: **215 passing** (+31 from the fix-prompts slice); core 3; leaderboard 31. Web repo: **274 passing** (+14 from `leaderboard-fetch.test.ts` and `leaderboard-page.test.tsx`; 2 pre-existing failures inherited from `dev` in `run-detail-scan-failure.test.tsx` / `installation-reconcile.test.ts` / `webhook-installation.test.ts` are unrelated to this slice). |
| Lint | clean (Biome) |
