...
**Last updated:** 2026-06-16 by Claude Code (web `admin-panel` slice) — **Platform admin panel live in production (ADR-0037) + all Dependabot alerts cleared (ADR-0038).** New `/admin` surface (web), gated by `user.role === 'admin'`: 3-layer gate (middleware cookie check → layout role check `notFound()`/404, never 403/redirect → server actions re-verify), `role` is `input:false` in Better-Auth (no API can set it; admin granted only by manual DB mutation), column allowlist blocks secret columns, admin nav link server-rendered for admins only. Phase 1: read-only users/orgs/subscriptions + support tickets (`/support` for users, `/admin/tickets` for staff) + revoke-sessions + `admin_audit_log`. Phase 2: delete user/org + ban/unban, each with a typed confirmation (re-type email/slug) and audited; ban enforced via a `session.create.before` hook. Schema migrations 0006 + 0007 applied to BOTH Neon branches (prod migrated before the main deploy so code never read a missing `role` column). Also fixed pre-existing prod schema drift (`org_policy` + `scan_failure` were missing from the prod branch). **Dependabot: 8 → 0** — better-auth 1.2.7→1.6.18, drizzle-orm 0.36.4→0.45.2 (SQL-injection fix) + drizzle-kit 0.31.10, tar 7.5.16, exact pnpm-workspace overrides for shell-quote 1.8.4 (critical)/vite 8.0.16/esbuild 0.28.1/qs 6.15.2, removed deprecated `@types/tar`. Web tests **461 → 492**; typecheck + build green. Web PRs #39 (dev, merged) + dev→main. CLI untouched (charter §3).

**Last updated:** 2026-06-17 by Claude Code (web `og-social` slice) — **Social previews + public per-repo score pages live (ADR-0039).** The root metadata pointed at a `/og.png` that never existed — links unfurled bare. Fixed: static OG cards for landing/pricing/leaderboard (`public/*.png`) + per-route metadata, AND a new public route `/r/[owner]/[name]` (no auth, public-run scores only — same resolution rule as the badge) whose `next/og` `opengraph-image.tsx` renders a dynamic card "owner/name · NN/100" color-coded by score (green ≥80 / amber ≥50 / red <50). Every scanned repo now has a shareable URL that unfurls with its live score. `lib/repo-score.ts` resolves the score. Verified `200 image/png` on preview AND prod (Satori renders at request time, not build) + visual check of the dynamic card. Web PRs #47 (dev) → #48 (main). typecheck/build green, **497 web tests** pass, Biome clean. **Marketing assets shipped alongside (owner-driven launch — charter §3.2 respected: agent drafted, owner published):** X account **@agentlintsh** launched with a 4-tweet thread; a 26.5s HyperFrames intro video (`marketing/video/` — CLI-terminal sim + app showcase + music) attached to the launch tweet. CLI untouched.

**Last updated:** 2026-06-16 by Claude Code (web `observability` slice) — **Sentry + PostHog live in production (ADR-0036).** Web app now has error/performance monitoring (Sentry @sentry/nextjs v10: instrumentation server/edge/client + global-error, source-map upload, `/monitoring` tunnel) and product analytics (PostHog us-cloud: autocapture ON, session replay OFF, manual App-Router pageviews, identify on sign-in, reverse-proxy via `/ingest` rewrites). All env-gated (absent keys = disabled). Privacy policy updated — the old "no third-party tracking cookies / no telemetry to any third party" lines were now false; PostHog + Sentry disclosed and added as sub-processors. Verified end-to-end on preview AND prod via headed Playwright: PostHog events POST to `/ingest/e/` 200, Sentry errors tunnel through `/monitoring` 200, SDK v10.57.0 init confirmed. Web PRs #37 (dev) + #38 (main). 461 tests unchanged. CLI untouched (charter §3).

**Last updated:** 2026-06-12 by Claude Code (web `v3-redesign` slice) — **Web v3 shipped to production.** agentlint.sh redesigned end-to-end (web PR #34/#35/#36): landing renders a real CLI 2.3.0 demo report with the same components the dashboard uses; new `/docs` (5 guides) and `/compare` (Codecov / SonarQube-class / manual checklists); pricing collapsed to a **single Pro plan at $5/mo flat per org** (ADR-0035) — legacy `team` rows normalise to `pro`, owner bypass removed; Stripe $5 prices live (test `price_1ThNiO9F4iHrjiRHDQuVEccH`, live `price_1ThNjc8ZW1aXed7SD7gt4UXe`); dashboard gains a zero-dep SVG chart kit (ScoreDial/CategoryBars/TrendChart/Sparkline) and run pages surface `fix.prompt` — consolidated copyable prompt panel mirroring `agentlint prompt`. Scan worker migrated to the stable `runScan` API (cli 2.1→2.3 deep imports had broken, as ADR-0032 predicted). Fixed prod checkout 500: `customer_update {name,address}=auto` required with `tax_id_collection` + existing customer. Web tests 274 → **461**. Verified on preview via headed-browser E2E incl. full test-mode checkout → webhook → Pro entitlement unlock. Known debt: web repo Biome baseline fails (~15k format errors predating this slice).

**Last updated:** 2026-06-11 by Claude Code (`fix-prompts` slice) — **Predefined fix prompts shipped (ADR-0034).** Every rule now has a static, no-LLM fix-prompt template (`packages/cli/src/prompts/registry.ts`, completeness-tested); `runScan` attaches them to actionable results as `fix.prompt`; new `agentlint prompt [path] [--rule <id>]` subcommand prints one consolidated, copy-pasteable markdown prompt (fails first, weight-descending, anti-gaming ground rules, verification footer). Markdown/HTML/JSON reporters surface prompts; terminal hints at the subcommand. New repo-shipped agent skill `skills/agentlint/SKILL.md` (audit → fix → verify; installed binary or `npx -y @agentlinthq/cli@latest` fallback). CLI tests 184 → **215** (+31). Scoring contract untouched. **Released 2026-06-12:** `@agentlinthq/cli@2.3.0` + `@agentlinthq/core@1.1.0` on npm (PR #21, tag v2.3.0, GitHub Release published, `pnpm dlx` smoke-tested).
...
| Latest commit (CLI) | [`#21`](https://github.com/agentlint/agentlint/pull/21) `release(cli): v2.3.0 — fix prompts; core v1.1.0` (2026-06-12). Prior: [`#20`](https://github.com/agentlint/agentlint/pull/20) fix prompts + `agentlint prompt` + agent skill (ADR-0034). |
| Self-audit | CLI repo: 100/100. Web repo: typecheck clean, build green. |
| Tests | CLI repo: **215 passing** (+31 from the fix-prompts slice); core 3; leaderboard 31. Web repo: **492 passing** (+31 from the admin-panel slice: `admin-access`, `admin-queries`, `admin-tickets`, `admin-mutations`, nav gating). |
| Lint | clean (Biome) |
...
| Web repository | ✅ https://github.com/agentlint/agentlint.sh (**private**). GH Free can't protect private main — fallback: `.githooks/pre-push` + `branch-policy.yml` CI flag (ADR-0021). Dependabot: **0 open** (cleared 2026-06-16, ADR-0038). |
| npm package | ✅ [`@agentlinthq/cli@2.3.0`](https://www.npmjs.com/package/@agentlinthq/cli) (latest, published 2026-06-12 — fix prompts + `prompt` subcommand). [`@agentlinthq/core@1.1.0`](https://www.npmjs.com/package/@agentlinthq/core) (additive `Result.fix.prompt`). |
| GitHub Release | ✅ [v2.3.0](https://github.com/agentlint/agentlint/releases/tag/v2.3.0) (latest), [v2.1.0](https://github.com/agentlint/agentlint/releases/tag/v2.1.0), [v2.0.0](https://github.com/agentlint/agentlint/releases/tag/v2.0.0), [v1.1.0](https://github.com/agentlint/agentlint/releases/tag/v1.1.0), [v1.0.0](https://github.com/agentlint/agentlint/releases/tag/v1.0.0) |
